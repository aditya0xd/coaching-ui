    ...
    <section className="container mx-auto px-5 md:px-12 py-16 md:py-32 max-w-7xl">
      <div className="bg-card border border-border rounded-[24px] md:rounded-[48px] overflow-hidden shadow-2xl flex flex-col lg:flex-row lg:items-stretch">
        ...
        >
          ...
          `} />
          <div className={`relative w-full max-w-[280px] md:max-w-[360px] aspect-[3/4] shadow-2xl transition-transform duration-700
          ...
          {!submitted ? (
            <>
              <div className="space-y-6">
                ...
                <p className="text-muted-foreground text-lg md:text-xl lg:text-2xl leading-relaxed max-w-3xl whitespace-normal font-medium">
                  Stop guessing and start seeing results. Grab the exact system I use with my high-level clients to kickstart their transformation.
                </p>
              </div>
              ...
              <ul className="grid grid-cols-1 gap-4">
                {[
                  ...
                  <li key={i} className="flex items-center gap-4 text-foreground/90">
                    ...
                    <span className="text-base md:text-lg lg:text-xl font-semibold leading-snug">{item}</span>
                  </li>
                ))}
              </ul>
              ...
              <div className="pt-6">
                <form onSubmit={handleSubmit} className="flex flex-col md:flex-row gap-4 w-full max-w-2xl">
                  ...
                  >
                    ...
                    {!loading && <Download className="w-5 h-5 ml-2 transition-transform group-hover:translate-y-1" />}
                  </Button>
                </form>
                <div className="mt-5 flex flex-wrap items-center justify-center md:justify-start gap-5 text-xs md:text-sm text-muted-foreground/60 font-medium">
                  <span className="flex items-center gap-1.5"><CheckCircle2 className="w-4 h-4 text-accent/50" /> No Credit Card</span>
                  <span className="flex items-center gap-1.5"><CheckCircle2 className="w-4 h-4 text-accent/50" /> Instant Access</span>
                </div>
              </div>
            </>
          ) : (
            <div className="text-center space-y-6 md:space-y-8 py-10 md:py-20 animate-in fade-in zoom-in slide-in-from-bottom-4 duration-700">
              ...
              <div className="space-y-3">
                <h3 className="text-3xl md:text-4xl font-extrabold text-foreground">Check Your Inbox!</h3>
                <p className="text-muted-foreground text-lg md:text-xl max-w-md mx-auto leading-relaxed">
                  The Blueprint is on its way to <span className="font-bold text-foreground underline decoration-accent/30 decoration-2 underline-offset-4">{email}</span>. 
                </p>
