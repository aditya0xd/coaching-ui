...
export function Problem() {
  ...
  return (
    <section className="bg-slate-950 min-h-screen container mx-auto px-5 md:px-12 py-24 border-y border-border/30">
      <div className="max-w-4xl mx-auto flex flex-col lg:flex-row items-center gap-12 lg:gap-24">
        <div className="flex-1 space-y-6">
          ...
          <p className="text-muted-foreground/80 text-lg md:text-xl leading-relaxed">
            Most people fail not because they lack willpower, but because they
            lack a sustainable system designed for their busy lives.
          </p>
          <ul className="space-y-4">
            {problems.map((p, i) => (
              <li
                key={i}
                className="flex items-center gap-3 text-foreground/70 font-medium text-base md:text-lg"
              >
                <XCircle
                  className="w-5 h-5 text-accent/40 flex-shrink-0 mt-1"
                  aria-hidden="true"
                />
                {p}
              </li>
        ...
        <div className="flex-1 w-full bg-muted/10 rounded-2xl p-8 md:p-12 border border-border/50 relative overflow-hidden group">
          <h3 className="text-xl md:text-2xl font-bold text-foreground/90 mb-4">
            Why most people get stuck
          </h3>
          <p className="text-muted-foreground/80 text-base leading-relaxed mb-6">
            Relying on "motivation" and extreme restrictions is a recipe for
            burnout. You need a system that works <strong>around</strong> your
            career and family, not against it.
          </p>
